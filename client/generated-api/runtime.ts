...
import { IAccessProvider } from "../services/http-base";
...
export class BaseAPI {
    ...
    protected request<T>(requestOpts: RequestOpts, responseOpts?: ResponseOpts): Observable<T | RawAjaxResponse<T>> {
        return from(this.createRequestArgs(requestOpts)).pipe(switchMap(args => this.rxjsRequest(args)),
            map((res) => {
    ...
    private createRequestArgs = async ({ url: baseUrl, query, method, headers, body, responseType }: RequestOpts): Promise<RequestArgs> => {
        // only add the queryString to the URL if there are query parameters.
        // this is done to avoid urls ending with a '?' character which buggy webservers
        // do not handle correctly sometimes.
        const url = `${this.baseUrl}${baseUrl}${query && Object.keys(query).length ? `?${queryString(query)}`: ''}`;
